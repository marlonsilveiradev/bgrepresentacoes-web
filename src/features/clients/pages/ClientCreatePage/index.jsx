// // ============================================================
// // src/features/clients/pages/ClientCreatePage/index.jsx
// //
// // Endpoint: POST /onboarding  (sempre multipart/form-data)
// //   campo "data"       = JSON.stringify(textPayload)
// //   campo "contrato"   = File (company_document)
// //   campo "documentos" = File[] (endereço → banco → maquininha)
// //
// // Modos de contratação:
// //   PLANO      → plan_id obrigatório, bandeiras fixas (preview)
// //   INDIVIDUAL → flag_ids[] obrigatório (mín. 1), sem plan_id
// //
// // Otimizações:
// //   - Planos e bandeiras buscados uma única vez com cache em memória
// //   - CNPJ com debounce de 400ms para validação visual sem requisições
// //   - Yup com .transform() antes de .test() — mesma ordem do ClientEditPage
// // ============================================================
...
// // ── Caches em memória ─────────────────────────────────────────────────────────
// // Evita re-fetch toda vez que o usuário abre o formulário na mesma sessão
// let plansCache   = null;
// let flagsCache   = null;
// let partnersCache = null;

// // ── Schema Yup ────────────────────────────────────────────────────────────────
// // .transform() sempre ANTES de .test() — evita falso positivo em campo vazio

// const createSchema = yup.object({

//   // ── Dados obrigatórios (diferente do EditPage, aqui são editáveis) ─────────
//   corporate_name: yup
//     .string()
...
//   // ── Dados opcionais (iguais ao EditPage) ──────────────────────────────────
//   trade_name: yup
//     .string()
//     .nullable()
//     .transform((v) => v || null)
...
//   // Opcional mesmo com banco preenchido (decisão de negócio)
...
//         }}
//       />
//       <FieldHint>PDF, JPG ou PNG · máx. 3 MB</FieldHint>
//     </DocumentUploadCard>
//   );
// }
...
//   const [mode,           setMode]           = useState('plan');      // 'plan' | 'individual'
//   const [selectedPlanId, setSelectedPlanId] = useState('');
//   const [selectedFlags,  setSelectedFlags]  = useState([]);          // UUID[]
//   const [selectedFiles,  setSelectedFiles]  = useState({});          // { [slot.key]: File }

//   // ── CNPJ com display separado (máscara visual) ────────────────────────────
...
//   // ── Busca planos, bandeiras e parceiros (cache com invalidação) ──────────────
//   // Cache só é reaproveitado se tiver dados (length > 0).
//   // Se vier vazio (erro silenciado, rate limit, etc.), busca novamente.
//   // Cada requisição tem seu próprio .catch() para não bloquear as demais.
...
//           // Bandeiras — invalida cache vazio
...
//           // Parceiros — falha silenciosa (opcional no formulário)
...
//   // ── Handler CNPJ com debounce (máscara visual sem requisição) ─────────────
...
//       // Atualiza o valor no react-hook-form apenas com dígitos
//       setValue('cnpj', onlyDigits(raw), { shouldValidate: true });
//     }, 400);
//   };

//   // ── Cálculo do total (modo individual) ────────────────────────────────────
...
//   // Plano selecionado (para exibir o preview de bandeiras e preço)
...
//         // Plano ou bandeiras individuais (mutuamente exclusivos)
//         ...(mode === 'plan'
...
//       // Sempre multipart/form-data para o onboarding
...
//       const { data } = await api.post('/onboarding', formDataObj, {
//         headers: { 'Content-Type': 'multipart/form-data' },
//       });

//       toast.success(
//         `Cliente cadastrado com sucesso! Protocolo: ${data.data?.client?.protocol ?? ''}`,
//         { autoClose: 5000 }
//       );

//       // Redireciona para o detalhe do cliente recém-criado
...
//                   $hasError={!!err('state_registration')}
//                   disabled={isSubmitting}
...
//                   ))}
//                 </Select>
//                 <FieldHint>{partners.length} parceiro{partners.length !== 1 ? 's' : ''} disponível{partners.length !== 1 ? 'is' : ''}</FieldHint>
//               </Field>
...
//                 {plans.length === 0 ? (
//                   <EmptyState>
//                     <EmptyText>Nenhum plano ativo encontrado.</EmptyText>
//                   </EmptyState>
//                 ) : (
//                   <PlansGrid>
//                     {plans.map((plan) => {
...
//                             <PlanFlagsPreview>
//                               {plan.flags.map((f) => (
//                                 <PlanFlagTag key={f.id}>{f.name}</PlanFlagTag>
//                               ))}
//                             </PlanFlagsPreview>
//                           )}
//                         </PlanCard>
...
//                 {flags.length === 0 ? (
//                   <EmptyState>
//                     <EmptyText>Nenhuma bandeira ativa encontrada.</EmptyText>
//                   </EmptyState>
//                 ) : (
//                   <FlagsGrid>
//                     {flags.map((flag) => {
...
//                 {/* Total das bandeiras selecionadas */}
...
//           {isSubmitting
//             ? <><Spinner />Cadastrando…</>
//             : <><Save size={15} />Cadastrar Cliente</>}
//         </SaveButton>
//       </FormFooter>
//     </>
//   );
...
//
// Otimizações:
//   - Cache em memória para planos, bandeiras e parceiros
//   - CNPJ com debounce de 400ms
//   - Yup com .transform() antes de .test()
//   - Compressão de imagens (browser-image-compression) antes do upload
//   - apiUpload com timeout 120s e onUploadProgress
// ============================================================
